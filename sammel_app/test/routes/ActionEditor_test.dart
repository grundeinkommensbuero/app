  ...
  ***REMOVED***);
  ...
  group('shows all data', () {
    ...
    ***REMOVED***);
    ...
    {
      await show_action_editor(tester, termineSeiteWidget);
      ActionEditorState action_data = tester.state(find.byKey(Key('action editor')));
      action_data.setState(() {action_data.action.typ = TerminTestDaten.einTermin().typ; action_data.validateInput();***REMOVED***);
      await tester.pump();
      expect(find.descendant(of: find.byType(ActionEditor), matching: find.text("Sammeln")), findsOneWidget);
    ***REMOVED***);


    testWidgets('Type dialog shows correct no typ', (WidgetTester tester) async
    {
      await show_action_editor(tester, termineSeiteWidget);
      ActionEditorState action_data = tester.state(find.byKey(Key('action editor')));
      action_data.setState(() {action_data.action.typ = ''; action_data.validateInput();***REMOVED***);
      await tester.pump();
      expect(find.descendant(of: find.byType(ActionEditor), matching: find.text("Wähle die Art der Aktion")), findsOneWidget);
    ***REMOVED***);
    ...
    ***REMOVED***);
    ...
    testWidgets('Zeitraum is correctly shown', (WidgetTester tester) async
    {
      ...
      action_data.setState(() {action_data.action.von = von; action_data.action.bis = bis;
                               action_data.validateInput();***REMOVED***);
      await tester.pump();
    ...
    ***REMOVED***);
    ...
    testWidgets('No Zeitraum is correctly shown', (WidgetTester tester) async
    {
      await show_action_editor(tester, termineSeiteWidget);
      ActionEditorState action_data = tester.state(find.byKey(Key('action editor')));
      action_data.setState(() {action_data.action.von = null; action_data.action.bis = null;
      action_data.validateInput();***REMOVED***);
      await tester.pump();
    ...
    ***REMOVED***);
    ...
    testWidgets('Location dialog are correctly shown', (WidgetTester tester) async
    {
      await show_action_editor(tester, termineSeiteWidget);
      ActionEditorState action_data = tester.state(find.byKey(Key('action editor')));
      expect(find.text("in Görlitzer Park und Umgebung"), findsNothing);
      when(stammdatenService.ladeOrte()).thenAnswer((_) async => [goerli()]);
      action_data.setState(() {action_data.action.ort = goerli();***REMOVED***);
      await tester.pump();
      expect(find.text("in Görlitzer Park und Umgebung"), findsOneWidget);
    ***REMOVED***);

    testWidgets('changing kontakt is correctly shown', (WidgetTester tester) async
    {
      await show_action_editor(tester, termineSeiteWidget);
      ActionEditorState action_data = tester.state(find.byKey(Key('action editor')));
      action_data.setState(() {action_data.action.terminDetails.kontakt = 'test1';***REMOVED***);
      await tester.pump();
      expect(find.text('test1'), findsOneWidget);
    ***REMOVED***);

    testWidgets('changing treffpunkt is correctly shown', (WidgetTester tester) async
    {
      await show_action_editor(tester, termineSeiteWidget);
      ActionEditorState action_data = tester.state(find.byKey(Key('action editor')));
      action_data.setState(() {action_data.action.terminDetails.treffpunkt = 'test1';***REMOVED***);
      await tester.pump();
      expect(find.text('Treffpunkt: test1'), findsOneWidget);
    ***REMOVED***);

    testWidgets('changing beschreibung is correctly shown', (WidgetTester tester) async
    {
      await show_action_editor(tester, termineSeiteWidget);
      ActionEditorState action_data = tester.state(find.byKey(Key('action editor')));
      action_data.setState(() {action_data.action.terminDetails.kommentar = 'test1';***REMOVED***);
      await tester.pump();
      expect(find.text('Beschreibung: test1'), findsOneWidget);
    ***REMOVED***);

    testWidgets('changing tage is correctly shown', (WidgetTester tester) async
    {
      await show_action_editor(tester, termineSeiteWidget);
      ActionEditorState action_data = tester.state(find.byKey(Key('action editor')));
      action_data.setState(() {action_data.action.tage = [DateTime(2019, 12, 1)];***REMOVED***);
      await tester.pump();
      expect(find.text('am 01.12.,'), findsOneWidget);
    ***REMOVED***);

    testWidgets('no tage selection is correctly shown', (WidgetTester tester) async
    {
      await show_action_editor(tester, termineSeiteWidget);
      ActionEditorState action_data = tester.state(find.byKey(Key('action editor')));

      action_data.setState(() {action_data.action.tage = List<DateTime>(); action_data.validateInput();***REMOVED***);
      await tester.pump();
    ...
    ***REMOVED***);
    ...
    testWidgets('All widgets show the correct action data',
          ...
          expect(find.text('Beschreibung: Bringe Westen und Klämmbretter mit'), findsOneWidget);
          expect(find.text('am 04.11.,'), findsOneWidget);
        ***REMOVED***);

    testWidgets('Cancel button closes correctly',
            (WidgetTester tester) async {
          await show_action_editor(tester, termineSeiteWidget);
    ...
    testWidgets('Termin adds a day if bis before von',
          ...
          await show_action_editor(tester, termineSeiteWidget);
          ActionEditorState action_data = tester.state(find.byKey(Key('action editor')));
          action_data.setState(() {action_data.action.tage = [DateTime(2019, 12, 1)]; action_data.action.von = TimeOfDay(hour: 10, minute: 32);
          action_data.action.bis = TimeOfDay(hour: 10, minute: 31); action_data.validateInput();***REMOVED***);
