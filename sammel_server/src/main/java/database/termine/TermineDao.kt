...
open class TermineDao {
    ...
    ***REMOVED***
    ...
    open fun erzeugeGetTermineQuery(filter: TermineFilter): TypedQuery<Termin> {
        ...
        if(filter.tage.isNotEmpty()) query.setParameter("tage", filter.tage.map { it.toDate() ***REMOVED***)
        ...
        if(filter.orte.isNotEmpty()) query.setParameter("orte", filter.orte.map { it.convertToOrt() ***REMOVED***)
