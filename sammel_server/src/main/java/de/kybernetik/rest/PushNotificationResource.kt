...
open class PushNotificationResource {
    ...
    open fun pushToParticipants(nachricht: PushMessageDto, @PathParam("actionId") actionId: Long): Response? {
        ...
        LOG.debug("Pushe Nachricht an ${firebaseKeys.size} Firebase-Keys und ${empfaengerOhneFirebase.size} weitere Benutzer")
    ...
    open fun pullNotifications(): Response? {
        ...
        val pushMessages =
                pushMessageDao.ladeAllePushMessagesFuerBenutzer(userId)
                        .map { PushMessageDto.convertFromPushMessage(it) }
        LOG.debug("${pushMessages.size} PushMessages für Benutzer $userId geladen")
