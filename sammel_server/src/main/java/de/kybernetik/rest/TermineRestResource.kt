...
open class TermineRestResource {
    ...
    open fun getTermine(filter: TermineFilter?): Response {
        LOG.debug("Lade Aktionen mit Filter ${filter?.typen}, ${filter?.tage}, ${filter?.von}, ${filter?.bis}, ${filter?.orte}, ${filter?.nurEigene}")
    ...
    open fun legeNeuenTerminAn(actionAndToken: ActionWithTokenDto): Response {
        LOG.info("Lege neue Aktion an in ${actionAndToken.action?.ort} durch ${context.userPrincipal.name}")
