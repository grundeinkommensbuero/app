...
class PushMessageDaoTest {
    ...
    ***REMOVED***
    ...
    fun `ladeAllePushMessagesFuerBenutzer sucht nach Benutzer-ID`() {
        val pushMessages = listOf(PushMessage(karl(), emptyMap(), PushNotificationDto(channel = "Allgemein")))
    ...
    ***REMOVED***
    ...
    fun `speicherePushMessage legt PushMessage fuer jeden Empfaenger ab`() {
        dao.speicherePushMessageFuerEmpfaenger(PushNotificationDto(channel = "Allgemein"), mapOf("key" to "value"), listOf(karl(), rosa()))
    ...
    ***REMOVED***
    ...
    fun `speicherePushMessage kommt mit leerer Empfaengerliste klar`() {
        dao.speicherePushMessageFuerEmpfaenger(PushNotificationDto(channel = "Allgemein"), mapOf("key" to "value"), emptyList())
    ...
    ***REMOVED***
    ...
    fun `loeschePushMessages loescht Nachrichten in DB`() {
        val pushMessages = listOf(
                PushMessage(karl(), mapOf("key" to "value"), PushNotificationDto(channel = "Allgemein")),
                PushMessage(karl(), mapOf("key" to "value"), PushNotificationDto(channel = "Allgemein")),
                PushMessage(karl(), mapOf("key" to "value"), PushNotificationDto(channel = "Allgemein")))
